...
app.get("/src/dashboard-student", function (req, res) {
  if (req.session.loggedin) {
    connection.query(
      "SELECT full_name FROM userinfo WHERE prn=? AND password=?",
      [req.session.PRN, req.session.PASSWORD],
...
app.get("/src/grades", function (req, res) {
  ...
  connection.query(sql, [req.session.PRN], function (error, results) {
    ...
    if (results.length == 0) {
      res.send(
        `Grades have not filled by admin yet.<br><a href="/src/dashboard-student">Go back to dashboard.</a>`
      );
    } else {
...
app.post("/src/register.html", function (req, res) {
  ...
  if (password.length > 7 && contact.length == 10) {
    connection.query(
      "SELECT COUNT(*) AS cnt FROM userinfo WHERE email = ?",
      [email],
      function (error, results) {
        if (error) {
          console.log(err);
          ...
          if (results[0].cnt > 0) {
            res.end(
              `This email is already in use, try with another email. <br><a href="/src/register.html">Click me to register again..</a>`
            );
          } else {
            connection.query(
              "SELECT COUNT(*) AS cnt FROM userinfo WHERE prn = ?",
              [prn],
              function (error, results) {
                if (error) {
                  console.log(err);
                  ...
                  if (results[0].cnt > 0) {
                    res.end(
                      `This prn is already in use, try with another prn. <br><a href="/src/register.html">Click me to register again..</a>`
                    );
                  } else {
                    var sql =
                      "INSERT INTO userinfo (full_name ,address, email, prn, password, contact, course) VALUES ('" +
                      name +
                      "','" +
                      address +
                      "','" +
    ...
    res.end(
      `Password must be at least 8 characters and contact number must be 10 characters long ..<br><a href="/src/register.html">Click me to register again..</a>`
    );
  }
});
...
app.post("/authlogin", function (req, res) {
  ...
  if (prn == 11 && password == "admin11") {
    ...
    connection.query(
      "SELECT * FROM userinfo WHERE prn=? AND password=?",
      [prn, password],
      function (error, results, fields) {
        ...
        if (results.length > 0) {
          ...
          res.send(
            `PRN or password is incorrect. Please enter correct values. <br><a href="/src/login.html">Click here to login again.</>`
          );
          //res.redirect('/src/login.html');
        }
      }
...
app.post("/src/contact.html", function (req, res) {
  ...
  var sql =
    "INSERT INTO feedback (fname, lname, contact, email, feedback) VALUES('" +
    fname +
    "','" +
    lname +
    "','" +
  ...
  connection.query(sql, function (error, results) {
    if (error) throw error;
    res.end(
      'Your feedback is reported successfully!! <br>Return to <a href="/src/index.html">Homepage</a>'
    );
  });
});
...
app.get("/src/dashboard-admin", function (req, res) {
  var sql =
    "INSERT INTO grades(prn, name) SELECT DISTINCT prn, full_name FROM userinfo WHERE prn NOT IN( SELECT prn FROM grades)";
  connection.query(sql, function (error, results) {
    if (error) throw error;
...
app.get("/src/update-student", function (req, res) {
  ...
  if (prn != 11) {
    ...
    res.end(
      `You can't update admin !! <br><a href="/src/update">Go back to update  student.</a>`
    );
  }
});
...
app.post("/src/update-student", function (req, res) {
  ...
  var sql =
    "UPDATE userinfo SET full_name = ?, address = ?, email = ?,prn = ?, password = ?, contact = ?, course = ? WHERE prn = ?";
  connection.query(
    sql,
    [name, address, email, prn, password, contact, course, prn],
    function (error, results) {
      if (error) throw error;
...
app.get("/src/delete-student", function (req, res) {
  ...
  if (prn != 11) {
    ...
    res.end(
      `You can't delete admin !! <br><a href="/src/update">Go back to update  student.</a>`
    );
  }
});
...
app.get("/src/grades-form", function (req, res) {
  ...
  if (prn != 11) {
    ...
    res.send(
      `You can't give grades to admin !!<br><a href="/src/fill-grades">Go back to fill grades panel .</a>`
    );
  }
});
...
app.post("/src/grades-form", function (req, res) {
  ...
  connection.query(
    ...
    function (error, results) {
      if (error) throw error;
      res.send(
        `Data stored successfully !!<br><a href="/src/dashboard-admin">Return to dashboard.</a>`
      );
    }
  );
});
...
app.post("/src/send-messages", function (req, res) {
  ...
  connection.query(sql, function (error, results) {
    if (error) throw error;
    res.send(
      `Message have been sent !!<br><a href="/src/dashboard-admin">Go back to Dashboard. </a>`
    );
  });
});
...
app.get("/src/attendance", function (req, res) {
  let sql = "SELECT prn, full_name from sis.userinfo ORDER BY prn ASC;";
  connection.query(sql, function (error, results) {
    if (error) throw error;
    res.render(__dirname + "/src/attendance", { info: results });
  });
});

app.post("/mark-attendance", function (req, res) {
  let presenti = req.body.present;
  let absenti = req.body.absent;
  console.log(presenti, absenti);
});
